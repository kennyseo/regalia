<div class="main-header navbar-default">
    <div class="container">
        <div class="row header-row">
            <div class="col-md-2 logo-wrap">
                <style>
.logo {
z-index: 9999;
}
</style>
<a id="logo" class="logo" href="https://shop.uofastore.com/default.aspx" title="University of Arkansas Bookstore">
<img alt="" src="https://shop.uofastore.com/skins/skin_1/images/desktop-logo.png" class="hidden-xs hidden-sm center-block store-logo" width="175px" />
<img alt="" src="https://shop.uofastore.com/skins/skin_1/images/logo-mobile.png" class="img-responsive visible-xs visible-sm center-block store-logo" />
</a>
            </div>
            <div class="col-md-10 header-middle">

<div class="lead">The Bookstore Regalia Management System</div>

            </div>


        </div>
    </div>
</div>

<div class="top-navigation-wrap">
    <div class="container">
        <div class="row">
            <div class="col-md-2"></div>
            <div class="col-md-10">
                <div class="navbar navbar-default topnav" role="navigation">
                    <div class="navbar-header">
                        <button type="button" class="navbar-toggle" data-toggle="collapse" data-target="#top-nav-collapse">
                            <span class="sr-only">Toggle navigation</span>
                            <span class="icon-bar"></span>
                            <span class="icon-bar"></span>
                            <span class="icon-bar"></span>
                        </button>
                        <a class="navbar-brand visible-xs menu-label" data-toggle="collapse" data-target="#top-nav-collapse" href="#">
                            Menu
                        </a>
                    </div>

<div class="navbar-collapse collapse" id="top-nav-collapse">
  <ul class="nav navbar-nav">
    <li class="dropdown"><a class="dropdown-toggle clown" href="#" data-toggle="dropdown">Home</a></li>

    <li class="dropdown"><a class="dropdown-toggle clown" href="#" data-toggle="dropdown">Students</a>
      <div class="dropdown-menu menu-students">

        <div class="row">
          <div class="col-md-6 col-sm-6 col-xs-12" style="border-left: 1px dotted #dedede;">
            <h4>Regalia Pickup Management</h4>
            <h5><a href="" class="nav-cate">Pick up Regalia</a></h5>
            <h5><a href="" class="nav-cate">Exchange Regalia</a></h5>
            <h5><a href="" class="nav-cate">Update Grad/Regalia</a></h5>
            <h5><a href="" class="nav-cate">Add Graduate</a></h5>
          </div>
          <div class="col-md-6 col-sm-6 col-xs-12" style="border-left: 1px dotted #dedede;">
            <h4>Regalia Pickup List</h4>
            <h5><a href="" class="nav-cate">Find Graduates</a></h5>
            <h5><a href="" class="nav-cate">Graduate List</a></h5>
            <h5><a href="" class="nav-cate">Picked up List</a></h5>
            <h5><a href="" class="nav-cate">Pick up List by Dates</a></h5>
          </div>
        </div>
      </div>
    </li>




    <li class="dropdown"><a class="dropdown-toggle clown" href="#" data-toggle="dropdown">Faculty</a>
    <div class="dropdown-menu menu-faculty">
    <div class="row">
      <div class="col-md-6 col-sm-6 col-xs-12" style="border-left: 1px dotted #dedede;">
        <h4>Regalia Pickup Management</h4>
        <h5><a href="" class="nav-cate">Pick up Regalia</a></h5>
        <h5><a href="" class="nav-cate">Exchange Regalia</a></h5>
        <h5><a href="" class="nav-cate">Update Grad/Regalia</a></h5>
        <h5><a href="" class="nav-cate">Add Graduate</a></h5>
      </div>
      <div class="col-md-6 col-sm-6 col-xs-12" style="border-left: 1px dotted #dedede;">
        <h4>Regalia Pickup List</h4>
        <h5><a href="" class="nav-cate">Find Graduates</a></h5>
        <h5><a href="" class="nav-cate">Graduate List</a></h5>
        <h5><a href="" class="nav-cate">Picked up List</a></h5>
        <h5><a href="" class="nav-cate">Pick up List by Dates</a></h5>
      </div>
    </div>

    <!-- end of outreach centers div --> <!-- end of "research & outreach" dropdown menu -->
    <!-- end of "research & outreach" dropdown -->
    </div>
    </li>




    <li class="dropdown"><a class="dropdown-toggle clown" href="#" data-toggle="dropdown">Reports</a>
    <div class="dropdown-menu menu-reports">
    <h4>University's OFFICIAL RAZORBACK SHOP</h4>
    <div class="row">
      <div class="col-md-2 col-sm-2 col-xs-12" style="border-left: 1px dotted #dedede;">
      <h5><a href="http://shop.uofastore.com/c-89-men.aspx" class="nav-cate">Men</a></h5>
      <h5><a href="http://shop.uofastore.com/c-104-women.aspx" class="nav-cate">Women</a></h5>
      <h5><a href="https://shop.uofastore.com/c-125-kids.aspx" class="nav-cate">Kids</a></h5>
      <h5><a href="https://shop.uofastore.com/c-223-collections.aspx" class="nav-cate">Collections</a></h5>
      </div>
      <div class="col-md-2 col-sm-2 col-xs-12" style="border-left: 1px dotted #dedede;">
      <h5><a href="http://shop.uofastore.com/c-89-men.aspx" class="nav-cate">Men</a></h5>
      <h5><a href="http://shop.uofastore.com/c-104-women.aspx" class="nav-cate">Women</a></h5>
      <h5><a href="https://shop.uofastore.com/c-125-kids.aspx" class="nav-cate">Kids</a></h5>
      <h5><a href="https://shop.uofastore.com/c-223-collections.aspx" class="nav-cate">Collections</a></h5>
      </div>
      <div class="col-md-2 col-sm-2 col-xs-12" style="border-left: 1px dotted #dedede;">
      <h5><a href="http://shop.uofastore.com/c-89-men.aspx" class="nav-cate">Men</a></h5>
      <h5><a href="http://shop.uofastore.com/c-104-women.aspx" class="nav-cate">Women</a></h5>
      <h5><a href="https://shop.uofastore.com/c-125-kids.aspx" class="nav-cate">Kids</a></h5>
      <h5><a href="https://shop.uofastore.com/c-223-collections.aspx" class="nav-cate">Collections</a></h5>
      </div>
      <div class="col-md-2 col-sm-2 col-xs-12" style="border-left: 1px dotted #dedede;">
      <h5><a href="http://shop.uofastore.com/c-89-men.aspx" class="nav-cate">Men</a></h5>
      <h5><a href="http://shop.uofastore.com/c-104-women.aspx" class="nav-cate">Women</a></h5>
      <h5><a href="https://shop.uofastore.com/c-125-kids.aspx" class="nav-cate">Kids</a></h5>
      <h5><a href="https://shop.uofastore.com/c-223-collections.aspx" class="nav-cate">Collections</a></h5>
      </div>
      <div class="col-md-2 col-sm-2 col-xs-12" style="border-left: 1px dotted #dedede;">
      <h5><a href="http://shop.uofastore.com/c-89-men.aspx" class="nav-cate">Men</a></h5>
      <h5><a href="http://shop.uofastore.com/c-104-women.aspx" class="nav-cate">Women</a></h5>
      <h5><a href="https://shop.uofastore.com/c-125-kids.aspx" class="nav-cate">Kids</a></h5>
      <h5><a href="https://shop.uofastore.com/c-223-collections.aspx" class="nav-cate">Collections</a></h5>
      </div>
      <div class="col-md-2 col-sm-2 col-xs-12" style="border-left: 1px dotted #dedede;">
      <h5><a href="http://shop.uofastore.com/c-89-men.aspx" class="nav-cate">Men</a></h5>
      <h5><a href="http://shop.uofastore.com/c-104-women.aspx" class="nav-cate">Women</a></h5>
      <h5><a href="https://shop.uofastore.com/c-125-kids.aspx" class="nav-cate">Kids</a></h5>
      <h5><a href="https://shop.uofastore.com/c-223-collections.aspx" class="nav-cate">Collections</a></h5>
      </div>
    </div>

    <!-- end of outreach centers div --> <!-- end of "research & outreach" dropdown menu -->
    <!-- end of "research & outreach" dropdown -->
    </div>
    </li>



    <li class="dropdown"><a class="dropdown-toggle clown" href="#" data-toggle="dropdown">Admin</a>
    <div class="dropdown-menu menu-admin">
    <h4>University's OFFICIAL RAZORBACK SHOP</h4>
    <div class="row">
      <div class="col-md-2 col-sm-2 col-xs-12" style="border-left: 1px dotted #dedede;">
      <h5><a href="http://shop.uofastore.com/c-89-men.aspx" class="nav-cate">Men</a></h5>
      <h5><a href="http://shop.uofastore.com/c-104-women.aspx" class="nav-cate">Women</a></h5>
      <h5><a href="https://shop.uofastore.com/c-125-kids.aspx" class="nav-cate">Kids</a></h5>
      <h5><a href="https://shop.uofastore.com/c-223-collections.aspx" class="nav-cate">Collections</a></h5>
      </div>
      <div class="col-md-2 col-sm-2 col-xs-12" style="border-left: 1px dotted #dedede;">
      <h5><a href="http://shop.uofastore.com/c-89-men.aspx" class="nav-cate">Men</a></h5>
      <h5><a href="http://shop.uofastore.com/c-104-women.aspx" class="nav-cate">Women</a></h5>
      <h5><a href="https://shop.uofastore.com/c-125-kids.aspx" class="nav-cate">Kids</a></h5>
      <h5><a href="https://shop.uofastore.com/c-223-collections.aspx" class="nav-cate">Collections</a></h5>
      </div>
      <div class="col-md-2 col-sm-2 col-xs-12" style="border-left: 1px dotted #dedede;">
      <h5><a href="http://shop.uofastore.com/c-89-men.aspx" class="nav-cate">Men</a></h5>
      <h5><a href="http://shop.uofastore.com/c-104-women.aspx" class="nav-cate">Women</a></h5>
      <h5><a href="https://shop.uofastore.com/c-125-kids.aspx" class="nav-cate">Kids</a></h5>
      <h5><a href="https://shop.uofastore.com/c-223-collections.aspx" class="nav-cate">Collections</a></h5>
      </div>
      <div class="col-md-2 col-sm-2 col-xs-12" style="border-left: 1px dotted #dedede;">
      <h5><a href="http://shop.uofastore.com/c-89-men.aspx" class="nav-cate">Men</a></h5>
      <h5><a href="http://shop.uofastore.com/c-104-women.aspx" class="nav-cate">Women</a></h5>
      <h5><a href="https://shop.uofastore.com/c-125-kids.aspx" class="nav-cate">Kids</a></h5>
      <h5><a href="https://shop.uofastore.com/c-223-collections.aspx" class="nav-cate">Collections</a></h5>
      </div>
      <div class="col-md-2 col-sm-2 col-xs-12" style="border-left: 1px dotted #dedede;">
      <h5><a href="http://shop.uofastore.com/c-89-men.aspx" class="nav-cate">Men</a></h5>
      <h5><a href="http://shop.uofastore.com/c-104-women.aspx" class="nav-cate">Women</a></h5>
      <h5><a href="https://shop.uofastore.com/c-125-kids.aspx" class="nav-cate">Kids</a></h5>
      <h5><a href="https://shop.uofastore.com/c-223-collections.aspx" class="nav-cate">Collections</a></h5>
      </div>
      <div class="col-md-2 col-sm-2 col-xs-12" style="border-left: 1px dotted #dedede;">
      <h5><a href="http://shop.uofastore.com/c-89-men.aspx" class="nav-cate">Men</a></h5>
      <h5><a href="http://shop.uofastore.com/c-104-women.aspx" class="nav-cate">Women</a></h5>
      <h5><a href="https://shop.uofastore.com/c-125-kids.aspx" class="nav-cate">Kids</a></h5>
      <h5><a href="https://shop.uofastore.com/c-223-collections.aspx" class="nav-cate">Collections</a></h5>
      </div>
    </div>

    <!-- end of outreach centers div --> <!-- end of "research & outreach" dropdown menu -->
    <!-- end of "research & outreach" dropdown -->
    </div>
    </li>




    <li class="dropdown"><a class="dropdown-toggle clown" href="#" data-toggle="dropdown">Help</a>
    <div class="dropdown-menu menu-help">
    <h4>University's OFFICIAL RAZORBACK SHOP</h4>
    <div class="row">
      <div class="col-md-2 col-sm-2 col-xs-12" style="border-left: 1px dotted #dedede;">
      <h5><a href="http://shop.uofastore.com/c-89-men.aspx" class="nav-cate">Men</a></h5>
      <h5><a href="http://shop.uofastore.com/c-104-women.aspx" class="nav-cate">Women</a></h5>
      <h5><a href="https://shop.uofastore.com/c-125-kids.aspx" class="nav-cate">Kids</a></h5>
      <h5><a href="https://shop.uofastore.com/c-223-collections.aspx" class="nav-cate">Collections</a></h5>
      </div>
      <div class="col-md-2 col-sm-2 col-xs-12" style="border-left: 1px dotted #dedede;">
      <h5><a href="http://shop.uofastore.com/c-89-men.aspx" class="nav-cate">Men</a></h5>
      <h5><a href="http://shop.uofastore.com/c-104-women.aspx" class="nav-cate">Women</a></h5>
      <h5><a href="https://shop.uofastore.com/c-125-kids.aspx" class="nav-cate">Kids</a></h5>
      <h5><a href="https://shop.uofastore.com/c-223-collections.aspx" class="nav-cate">Collections</a></h5>
      </div>
      <div class="col-md-2 col-sm-2 col-xs-12" style="border-left: 1px dotted #dedede;">
      <h5><a href="http://shop.uofastore.com/c-89-men.aspx" class="nav-cate">Men</a></h5>
      <h5><a href="http://shop.uofastore.com/c-104-women.aspx" class="nav-cate">Women</a></h5>
      <h5><a href="https://shop.uofastore.com/c-125-kids.aspx" class="nav-cate">Kids</a></h5>
      <h5><a href="https://shop.uofastore.com/c-223-collections.aspx" class="nav-cate">Collections</a></h5>
      </div>
      <div class="col-md-2 col-sm-2 col-xs-12" style="border-left: 1px dotted #dedede;">
      <h5><a href="http://shop.uofastore.com/c-89-men.aspx" class="nav-cate">Men</a></h5>
      <h5><a href="http://shop.uofastore.com/c-104-women.aspx" class="nav-cate">Women</a></h5>
      <h5><a href="https://shop.uofastore.com/c-125-kids.aspx" class="nav-cate">Kids</a></h5>
      <h5><a href="https://shop.uofastore.com/c-223-collections.aspx" class="nav-cate">Collections</a></h5>
      </div>
      <div class="col-md-2 col-sm-2 col-xs-12" style="border-left: 1px dotted #dedede;">
      <h5><a href="http://shop.uofastore.com/c-89-men.aspx" class="nav-cate">Men</a></h5>
      <h5><a href="http://shop.uofastore.com/c-104-women.aspx" class="nav-cate">Women</a></h5>
      <h5><a href="https://shop.uofastore.com/c-125-kids.aspx" class="nav-cate">Kids</a></h5>
      <h5><a href="https://shop.uofastore.com/c-223-collections.aspx" class="nav-cate">Collections</a></h5>
      </div>
      <div class="col-md-2 col-sm-2 col-xs-12" style="border-left: 1px dotted #dedede;">
      <h5><a href="http://shop.uofastore.com/c-89-men.aspx" class="nav-cate">Men</a></h5>
      <h5><a href="http://shop.uofastore.com/c-104-women.aspx" class="nav-cate">Women</a></h5>
      <h5><a href="https://shop.uofastore.com/c-125-kids.aspx" class="nav-cate">Kids</a></h5>
      <h5><a href="https://shop.uofastore.com/c-223-collections.aspx" class="nav-cate">Collections</a></h5>
      </div>
    </div>

    <!-- end of outreach centers div --> <!-- end of "research & outreach" dropdown menu -->
    <!-- end of "research & outreach" dropdown -->
    </div>
    </li>

  </ul>
</div>
                    <!--/.nav-collapse -->
                </div>
            </div>
        </div>
    </div>
</div>
